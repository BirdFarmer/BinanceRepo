...
namespace BinanceTestnet.Indicators
{
    public static class ExpandingAverages
    {
        public static int CheckSMAExpansion(List<double> sma14, List<double> sma50, List<double> sma100, List<double> sma200, int index)
        {
            ...
            bool isUpwardExpansion = sma14[index] > sma50[index] && sma50[index] > sma100[index] && sma100[index] > sma200[index]
                && sma14[index - 2] < sma14[index] && sma50[index - 2] < sma50[index]
                && sma100[index - 2] < sma100[index] && sma200[index - 2] < sma200[index] 
                && (sma50[index] - sma50[index - 2]) > (sma100[index] - sma100[index - 2])
                && (sma100[index] - sma100[index - 2]) > (sma200[index] - sma200[index - 2]);

            bool isDownwardExpansion = sma14[index] < sma50[index] && sma50[index] < sma100[index] && sma100[index] < sma200[index]
                && sma14[index - 2] > sma14[index] && sma50[index - 2] > sma50[index]
                && (sma50[index - 2] - sma50[index]) > (sma100[index - 2] - sma100[index])
                && (sma100[index - 2] - sma100[index]) > (sma200[index - 2] - sma200[index]);
